...
class tx_ltubeprintpages_pi1 extends tx_ltubeprintpages_base {
	...
	public function main($content, array $conf) {
		...
		if(!$GLOBALS["TSFE"]->fe_user->user['uid']){ 
			return $this->pi_wrapInBaseClass($this->pi_getLL('login_error')); 
		}
		$this->pi_initPIflexForm();
		$this->_remove_http_and_last_slash();
		$this->_set_flexform_values_pi1();//print_r($this->pi1_flex_settings); die();
		if(!$this->pi1_flex_settings['storage_folder'] || !$this->pi1_flex_settings['root_page']) {
		...
		$content	= $this->_fetch_generate_save_content_as_html();
	...
	/**
	 * tx_ltubeprintpages_pi1::_fetch_generate_save_content_as_html
	 *
	 * @package
	 * @author Nirmalya
	 * @copyright Copyright (c) 2011
	 ...
	 * @access public
	 */
	function _fetch_generate_save_content_as_html(){
		if(!$this->pi1_pagestocreate){ return FALSE; }	
		$storage_folder	= $this->pi1_flex_settings['storage_folder']?$this->pi1_flex_settings['storage_folder']:$this->default_storage_folder;
		foreach($this->language_array as $language_uid=>$language_nm){		
			foreach($this->pi1_pagestocreate as $page_key=>$page_id){	//Foreach Pages
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('sys_language_uid,uid','tt_content','pid='.$page_id.' AND deleted = 0 AND hidden = 0 AND sys_language_uid='.$language_uid);				
				$page_content	= '<!--###'.$this->cut_marker.'###-->';
				while($row	= $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)){
					$conf			= array('tables' => 'tt_content','source' => $row['uid'],'dontCheckPid' => 1);    
					//$page_content	.= $this->cObj->cObjGetSingle('RECORDS', $conf);
					$page_content	.= $this->cObj->RECORDS($conf);
				}		
				$page_content	.= '<!--###'.$this->cut_marker.'###-->';
				$file_name		= $page_id.$this->file_suffix.$language_nm.'.html'; 
	...
	}//_fetch_generate_save_content_as_html
...
#---------------------------NOT IN USE-------------------------------------------------#
	...
	/**
	 * tx_ltubeprintpages_pi1::_login_user_automatically
	 *
	 * @package
	 * @author Nirmalya
	 * @copyright Copyright (c) 2011
...
/**
	...
	function _login_t3_site_via_curl_test(){
		$storage_folder	= $this->pi1_flex_settings['storage_folder']?$this->pi1_flex_settings['storage_folder']:$this->default_storage_folder;
		$base_url		= t3lib_div::getIndpEnv('TYPO3_SITE_URL');
		$file_suffix	= $this->file_suffix;
		...
		foreach($this->language_array as $language_key=>$language_id){	//Foreach languages		
			foreach($this->pi1_pagestocreate as $page_key=>$page_id){	//Foreach Pages
				$page_url	= $base_url."/index.php?id=".$page_id."&L=".$language_key; 
				$file_name	= $page_id.$file_suffix; 
				$file_path	= $storage_folder.'/'.$file_name.$language_id.'.html'; 
				$client = curl_init();
				curl_setopt($client, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($client, CURLOPT_FOLLOWLOCATION, true); 
				curl_setopt($client, CURLOPT_COOKIEJAR, 'cookies.txt');
				curl_setopt($client, CURLOPT_COOKIEFILE, 'cookies.txt');
				curl_setopt($client, CURLOPT_URL, $page_url);     
	...
	}//_login_t3_site_via_curl_test

	function getSingleContent($uid){
		$cObj = t3lib_div::makeInstance('tslib_cObj');
		$cObj->setParent($pi->cObj->data,$pi->cObj->currentRecord);
		$cObj->start($cObj->data, 'tt_content');
		$conf = array(
		  'table' => 'tt_content',
		  'select.' => array(
			'uidInList'   => $uid,
