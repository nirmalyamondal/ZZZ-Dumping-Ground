...
class TopicRepository extends \TYPO3\CMS\Extbase\Persistence\Repository {
    ...
    /**
     * Fetch getTopicStatus
     * 
     * @param integer $topicUid
     * @return void
     */
	function getTopicStatus($topicUid){
		$res	= $GLOBALS['TYPO3_DB']->exec_SELECTquery('hidden,deleted', 'tx_ltubetopicsystem_topic', 'uid='.intval($topicUid), '', '', 1);
		if (!$res) { return FALSE;	}
		$num_rows	= $GLOBALS['TYPO3_DB']->sql_num_rows($res);
		if($num_rows < 1){	return FALSE;	}
		$row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res);
		if(($row['hidden'] == 1) || ($row['deleted'] == 1)){ return FALSE; }
	return TRUE;
	}
